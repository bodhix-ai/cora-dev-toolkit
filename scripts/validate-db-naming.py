...
# SQL/PL/pgSQL keywords to exclude from column name validation
# These appear in function bodies, triggers, and RLS policies
SQL_KEYWORDS = {
    'select', 'from', 'where', 'and', 'or', 'not', 'in', 'exists', 'join',
    'left', 'right', 'inner', 'outer', 'on', 'using', 'as', 'case', 'when',
    'then', 'else', 'end', 'if', 'elsif', 'loop', 'while', 'for', 'return',
    'returns', 'declare', 'begin', 'exception', 'raise', 'notice', 'execute',
    'into', 'insert', 'update', 'delete', 'values', 'set', 'create', 'alter',
    'drop', 'table', 'index', 'function', 'trigger', 'before', 'after',
    'each', 'row', 'new', 'old', 'null', 'true', 'false', 'is', 'coalesce',
    'order', 'by', 'asc', 'desc', 'limit', 'offset', 'group', 'having',
    'distinct', 'all', 'any', 'some', 'union', 'intersect', 'except',
    'primary', 'key', 'foreign', 'references', 'constraint', 'unique',
    'check', 'default', 'cascade', 'restrict', 'security', 'definer',
    'language', 'plpgsql', 'sql', 'with', 'recursive', 'materialized',
    'view', 'policy', 'enable', 'disable', 'force', 'grant', 'revoke',
    'perform', 'found', 'strict', 'query', 'record', 'type', 'enum',
    'boolean', 'integer', 'text', 'varchar', 'uuid', 'timestamp', 'timestamptz',
    'date', 'time', 'interval', 'numeric', 'decimal', 'float', 'double',
    'precision', 'serial', 'bigserial', 'smallint', 'bigint', 'real',
    'jsonb', 'json', 'array', 'bytea', 'inet', 'cidr', 'macaddr',
}
...
# Standard CORA audit columns (not subject to FK _id rule)
# These columns reference user_profiles but use "by whom" naming convention
AUDIT_COLUMNS = {'created_by', 'updated_by', 'deleted_by', 'invited_by', 'edited_by'}
...
# Junction table patterns (Rule 3)
# Pattern: {entity1_singular}_{entity2_singular}
# These use singular nouns for both entities
JUNCTION_TABLE_PATTERNS = [
    # chat module
    'chat_session_kb',
    # Add more as needed: format is 'prefix_entity1_entity2'
]
...
def extract_column_name(line: str) -> Optional[str]:
    ...
    # Skip SQL keywords (found in function bodies, triggers, RLS policies)
...
def is_junction_table(table_name: str) -> bool:
    """Check if table follows junction table pattern (Rule 3)"""
    # Explicit junction table list
    if table_name in JUNCTION_TABLE_PATTERNS:
        return True
    
    # Heuristic: tables with two entity nouns (both singular) connected by underscore
    # Pattern: {prefix_}{entity1}_{entity2} where neither ends in 's'
    parts = table_name.split('_')
    if len(parts) >= 3:
        # Check if first part is a known prefix
        potential_prefix = parts[0] + '_'
        if potential_prefix in DOCUMENTED_PREFIXES:
            # Rest should be two singular entities
            rest = '_'.join(parts[1:])
            rest_parts = rest.split('_')
            if len(rest_parts) == 2:
                # Both parts should be singular (not ending in 's')
                if not is_plural(rest_parts[0]) and not is_plural(rest_parts[1]):
                    return True
...
def validate_table_name(table_name: str, result: ValidationResult, file: str, line_num: int):
    """Validate table naming (Rule 1, 2, 3, 6, 8)"""
    # Check if it's a core table (allowed)
    if table_name in CORE_TABLES:
        return
    
    # Check if it's a junction table (Rule 3 - singular nouns)
...
def validate_foreign_key(line: str, result: ValidationResult, file: str, line_num: int):
    ...
    # Skip CORA audit columns (created_by, updated_by, etc.)
    # These use "by whom" convention instead of "_id" convention
    if fk_column in AUDIT_COLUMNS:
        return
...
def validate_index_name(index_name: str, result: ValidationResult, file: str, line_num: int):
    ...
    # Skip SQL keywords that might be misdetected as index names
    if index_name.lower() in SQL_KEYWORDS:
        return
