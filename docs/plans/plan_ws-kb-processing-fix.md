...
**Related:** plan_ws-crud-kbs.md, PR #45 (doc-upload sprint)
...
**IN SCOPE:**
- Fix SQS trigger for workspace document uploads
- End-to-end document processing (parse → chunk → embed → store)
- Verify kb_chunks table population
- Verify embeddings generation with Bedrock Titan V2
...
- Processor: `DEFAULT_EMBEDDING_DIMENSION = 1024` ✅
...
SELECT document_id, array_length(embedding::real[], 1) as dim
FROM kb_chunks 
LIMIT 1;
```
...
- [ ] `kb_chunks.embedding` column contains 1024-dimension vector data
