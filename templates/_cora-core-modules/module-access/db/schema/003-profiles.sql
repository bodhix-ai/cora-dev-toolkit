...
-- Updated: December 20, 2025 - Renamed profiles → user_profiles
...
COMMENT ON TABLE public.user_profiles IS 'User profile information, one per auth.users record';
COMMENT ON COLUMN public.user_profiles.id IS 'Auto-incrementing BIGINT primary key';
COMMENT ON COLUMN public.user_profiles.user_id IS 'Foreign key to auth.users.id (one-to-one relationship)';
...
COMMENT ON COLUMN public.user_profiles.created_by IS 'User who created this profile (usually same as user_id for self-created)';
COMMENT ON COLUMN public.user_profiles.updated_by IS 'User who last updated this profile';
COMMENT ON COLUMN public.user_profiles.metadata IS 'Flexible JSONB storage for additional profile attributes';
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'user_profiles_current_org_id_fkey'
    ) THEN
        ALTER TABLE public.user_profiles 
        ADD CONSTRAINT user_profiles_current_org_id_fkey 
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'user_profiles_created_by_fkey'
    ) THEN
        ALTER TABLE public.user_profiles 
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'user_profiles_updated_by_fkey'
    ) THEN
        ALTER TABLE public.user_profiles 
