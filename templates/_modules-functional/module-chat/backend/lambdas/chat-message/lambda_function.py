...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if '/context' in path:
            ...
            if http_method == 'POST':
                return handle_get_rag_context(event, supabase_user_id, session_id)
        
        # History route: /chats/{sessionId}/history
        elif '/history' in path:
            session_id = path_params.get('sessionId')
            if http_method == 'GET':
                return handle_get_history(event, supabase_user_id, session_id)
        ...
        # Message routes: /chats/{sessionId}/messages...
            ...
            if http_method == 'GET':
                if message_id:
                    return handle_get_message(supabase_user_id, session_id, message_id)
                else:
                    return handle_list_messages(event, supabase_user_id, session_id)
            elif http_method == 'POST':
                return handle_send_message(event, supabase_user_id, session_id)
...
def handle_send_message(
    ...
    can_add = common.execute_rpc(
        function='can_add_messages',
        params={
            'p_user_id': user_id,
            'p_session_id': session_id
        }
...
def handle_get_rag_context(
    ...
    if kb_ids:
        ...
        grounded_kbs = common.execute_rpc(
...
def _search_kb_chunks(
    ...
    try:
        results = common.execute_rpc(
            function='search_kb_chunks',
            params={
                'p_kb_id': kb_id,
...
def _update_session_metadata(session_id: str, user_id: str) -> None:
    ...
    metadata['lastMessageAt'] = common.get_current_timestamp()
