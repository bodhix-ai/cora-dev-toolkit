...
-- ============================================================================
-- RLS POLICIES: eval_opt_run_executions (Sprint 6)
-- ============================================================================

-- Enable RLS
ALTER TABLE eval_opt_run_executions ENABLE ROW LEVEL SECURITY;

-- Policy: Users can view executions in workspaces they have access to
CREATE POLICY eval_opt_run_executions_select_policy
    ON eval_opt_run_executions
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM eval_opt_runs r
            WHERE r.id = eval_opt_run_executions.run_id
            AND EXISTS (
                SELECT 1 FROM ws_members wm
                WHERE wm.ws_id = r.ws_id
                AND wm.user_id = auth.uid()
            )
        )
    );

-- Policy: Users can create executions in workspaces they have access to
CREATE POLICY eval_opt_run_executions_insert_policy
    ON eval_opt_run_executions
    FOR INSERT
    WITH CHECK (
        EXISTS (
...
-- Policy: Users can update executions in workspaces they have access to
CREATE POLICY eval_opt_run_executions_update_policy
    ON eval_opt_run_executions
    FOR UPDATE
    USING (
        EXISTS (
...
-- Policy: Users can delete executions in workspaces they have access to
CREATE POLICY eval_opt_run_executions_delete_policy
    ON eval_opt_run_executions
    FOR DELETE
    USING (
        EXISTS (
