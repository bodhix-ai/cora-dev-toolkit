...
COMMENT ON COLUMN eval_opt_runs.proj_id IS 'Project foreign key';
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_eval_opt_run_proj 
    ON eval_opt_runs(proj_id);
CREATE INDEX IF NOT EXISTS idx_eval_opt_run_status 
    ON eval_opt_runs(proj_id, status);
...
CREATE INDEX IF NOT EXISTS idx_eval_opt_run_created_at 
    ON eval_opt_runs(proj_id, created_at DESC);
