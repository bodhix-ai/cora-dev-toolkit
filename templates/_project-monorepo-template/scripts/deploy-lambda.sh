...
list_lambdas() {
  ...
  # Find module Lambdas in packages/
  for module_dir in "${REPO_ROOT}/packages/module-"*; do
    if [[ -d "$module_dir/backend/lambdas" ]]; then
      module_name=$(basename "$module_dir")
      echo "  ${module_name}:"
      for lambda_dir in "${module_dir}/backend/lambdas/"*/; do
  ...
  if [[ -d "${REPO_ROOT}/lambdas/api-gateway-authorizer" ]]; then
    echo "    - authorizer"
  fi
...
if [[ "$LAMBDA_NAME" == "authorizer" ]]; then
  LAMBDA_DIR="${REPO_ROOT}/lambdas/api-gateway-authorizer"
  ZIP_NAME="authorizer.zip"
elif [[ "$LAMBDA_NAME" == *"/"* ]]; then
  # Module Lambda (e.g., module-access/invites) - in monorepo these are in packages/
  MODULE_NAME=$(echo "${LAMBDA_NAME}" | cut -d'/' -f1)
  LAMBDA_SHORT_NAME=$(echo "${LAMBDA_NAME}" | cut -d'/' -f2)
  LAMBDA_DIR="${REPO_ROOT}/packages/${MODULE_NAME}/backend/lambdas/${LAMBDA_SHORT_NAME}"
...
# --- In monorepo, packages are local ---
...
if [[ "$SKIP_BUILD" == "false" ]]; then
  ...
  if [[ "$LAMBDA_NAME" == *"module-"* ]]; then
    if [[ ! -d "${PACKAGES_DIR}" ]]; then
      ...
      log_info "Expected monorepo structure with packages/ at repo root"
      exit 1
    fi
    ...
    if [[ -f "${REPO_ROOT}/scripts/build-cora-modules.sh" ]]; then
      export SKIP_VALIDATION=true
      bash "${REPO_ROOT}/scripts/build-cora-modules.sh"
    else
      log_error "build-cora-modules.sh not found: ${REPO_ROOT}/scripts/build-cora-modules.sh"
      exit 1
    fi
  else
    # For non-module Lambdas (e.g., authorizer), build from lambdas/ directory
...
if [[ "$SKIP_UPLOAD" == "false" ]]; then
  ...
  if [[ -z "$S3_BUCKET" && -f "${REPO_ROOT}/envs/${ENVIRONMENT}/terraform.tfstate" ]]; then
    S3_BUCKET=$(cd "${REPO_ROOT}/envs/${ENVIRONMENT}" && terraform output -raw lambda_artifacts_bucket 2>/dev/null || true)
    if [[ -n "$S3_BUCKET" ]]; then
  ...
  if [[ -z "$S3_BUCKET" && -f "${REPO_ROOT}/envs/${ENVIRONMENT}/local-secrets.tfvars" ]]; then
    S3_BUCKET=$(grep 'lambda_bucket' "${REPO_ROOT}/envs/${ENVIRONMENT}/local-secrets.tfvars" | cut -d'"' -f2 || true)
    if [[ -n "$S3_BUCKET" ]]; then
      log_info "Using bucket from local-secrets.tfvars: ${S3_BUCKET}"
    fi
  fi
  
  # 5. Last resort: use default pattern (extract project name from repo root)
  if [[ -z "$S3_BUCKET" ]]; then
  ...
  # For module Lambdas, look in build directory or module's backend/.build
  if [[ "$LAMBDA_NAME" == *"module-"* ]]; then
    MODULE_NAME=$(echo "${LAMBDA_NAME}" | cut -d'/' -f1)
    LAMBDA_SHORT_NAME=$(echo "${LAMBDA_NAME}" | cut -d'/' -f2)
    
    # Try centralized build directory first
    if [[ -f "${REPO_ROOT}/build/${MODULE_NAME}/${LAMBDA_SHORT_NAME}.zip" ]]; then
      ZIP_PATH="${REPO_ROOT}/build/${MODULE_NAME}/${LAMBDA_SHORT_NAME}.zip"
    # Fallback to module's .build directory
    elif [[ -f "${REPO_ROOT}/packages/${MODULE_NAME}/backend/.build/${LAMBDA_SHORT_NAME}.zip" ]]; then
      ZIP_PATH="${REPO_ROOT}/packages/${MODULE_NAME}/backend/.build/${LAMBDA_SHORT_NAME}.zip"
    else
      log_error "Could not find zip file for ${LAMBDA_NAME}"
      log_info "Expected at: ${REPO_ROOT}/build/${MODULE_NAME}/${LAMBDA_SHORT_NAME}.zip"
      log_info "Or at: ${REPO_ROOT}/packages/${MODULE_NAME}/backend/.build/${LAMBDA_SHORT_NAME}.zip"
      exit 1
    fi
  else
